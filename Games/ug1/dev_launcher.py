...
if __name__ == '__main__':
    ...
    parser.add_option(
        ...
        dest="fullscreen",
        default=False,
        help="Run in fullscreen mode. Default is windowed mode.")
    parser.add_option(
        "-r",
        "--resolution",
        type="int",
        nargs=2,
        dest="res",
        help="Specify the resolution. Default is 0 0, which uses the screen's resolution.",
    ...
    parser.add_option(
        ...
        type="int",
        dest="fps",
        help="Specify the fps cap. Default is 30",
        metavar="FPS",
        default=30)
    parser.add_option(
        "-l",
        ...
        type="str",
        dest="locale",
        help="Specify the locale to launch the game with. Default uses the system's locale settings.",
        default="default")
    parser.add_option(
        "-t",
        "--list-translations",
        ...
        dest="list",
        default=False,
        help="List the available translations.")
    parser.add_option(
        "-p",
        "--profile",
        action="store_true",
        dest="profile",
        default=False,
        help="Enable profiling via cProfile. Individual profiles for each scene will be output. If graphviz is installed on your system, callgrind style output images will be created.")
    parser.add_option(
        "-o",
        "--output",
        type="string",
        dest="profile_output",
        default=None,
        help="Specify an output directory for profiling data. By default, output will be placed in profiles/%d-%m-%Y %H-%M-%S/")
    parser.add_option(
        "-g",
        "--graphviz",
        action="store_true",
        dest="graphviz",
        default=False,
        help="Use graphviz if available to generate png callgraphs if profiling is enabled.")
    (options, args) = parser.parse_args()

    languages = None
    if os.path.exists('./locale'):
        languages = [
            file for file in os.listdir('./locale') if os.path.isdir(
                './locale/%s' %
                file) and os.path.isfile(
                './locale/%s/activity.linfo' %
                file)]
    ...
    if options.locale != 'default':
        lang = gettext.translation(
            "org.laptop.community.cisc374.mathadder",
            "./locale",
            languages=[
                options.locale])
        lang.install()
