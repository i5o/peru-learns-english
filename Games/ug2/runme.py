...
class Tablero(spyral.View):
    ...
    def __init__(self, scene, topic, mapa):
        ...
        self.activar_snd = pygame.mixer.Sound(
            gamedir("sonidos/Retro_Game_Sounds_SFX_162.ogg"))
        self.match_snd = pygame.mixer.Sound(gamedir("sonidos/Powerup.ogg"))
        self.desactivar_snd = pygame.mixer.Sound(
            gamedir("sonidos/Randomize2.ogg"))
        self.win_snd = pygame.mixer.Sound(
            gamedir("sonidos/Retro_Game_Sounds_SFX_161.ogg"))
        ...
        if self.ACTIVADO:
            if (ubicacion.distance(self.ACTIVADO)) == 1.0:
                ...
                if "Nexo" in CANDIDATO.__class__.__name__:
                    ...
                    if "Nexo" in ANTERIOR.__class__.__name__:
                        ANTERIOR.ir_a(ubicacion)
                    INICIAL = self.tablero[
                        self.ACTIVADO_INICIAL.y][
                        self.ACTIVADO_INICIAL.x]
                    if CANDIDATO.COLOR == INICIAL.COLOR and (
                            CANDIDATO is not INICIAL):
...
class Cursor (spyral.Sprite):
    ...
    def down(self):
        self.ubicacion = spyral.Vec2D(
            self.ubicacion.x, (self.ubicacion.y + 1) %
            5)
        self.update()
    ...
    def update(self):
        ...
        self.desplaz_anim = spyral.Animation(
            "pos",
            QuadraticOutTuple(
                self.pos,
                newpos),
...
class Bloque (spyral.Sprite):
    ...
    def init_animations(self):
        ...
        self.blink_animation = spyral.Animation(
            "image",
            spyral.easing.Iterate(toda_secuencia),
            2)
...
class Escena(spyral.Scene):
    ...
    def intro2(self):
        self.intro = Dialogo(
            self,
            "Your mission is to match each pair of eyes in order to regenerate the water cycle.",
            self.start_now)
        self.titulo.kill()
    ...
    def win(self):
        ...
        win = Dialogo(
            self,
            "Congratulations! You've saved the forests!",
            self.win)
