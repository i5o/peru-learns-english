...
class CampodeEstrellas(spyral.Sprite):
    ...
    def __init__(self, scene):
        ...
        spyral.event.register(
            "CampodeEstrellas.demora.animation.end",
            self.spawn)
    ...
    def init_animations(self):
        self.top = 0.3
        self.low = 0.03
        self.slowdown_anim = spyral.Animation(
            "speed",
        ...
        self.speedup_anim = spyral.Animation(
            "speed",
            spyral.easing.QuadraticOut(
                self.low,
                self.top),
            duration=3)
        ...
        self.turnblue_alt_anim = (
            spyral.Animation(
                "R",
                spyral.easing.Linear(
                    128,
                    0),
                duration=2) & spyral.Animation(
                "G",
                spyral.easing.Linear(
                    192,
                    0),
        ...
        self.goodbye_anim = (
            spyral.Animation(
                "R", spyral.easing.Linear(
                    0, 0), duration=3) & spyral.Animation(
                "G", spyral.easing.Linear(
                    0, 0), duration=5) & spyral.Animation(
                        "B", spyral.easing.Linear(
                            40, 80), duration=5))
    ...
    def init_stars(self):
        ...
        for i in range(self.num_stars):
            ...
            star = [randrange(-
                              25, 25), randrange(-
                                                 25, 25), randrange(1, self.max_depth)]
...
class Bloque (spyral.Sprite):
    ...
    def render_image(self, image):
        try:
            nueva = spyral.Image(
                filename=image).scale(
                (self.image.width -
                 self.margin,
                 self.image.height -
                 self.margin))
        except ValueError as xxx_todo_changeme:
            pygame.error = xxx_todo_changeme
            nueva = spyral.Image(
                size=(
                    self.width -
                    self.margin,
...
class Nave (spyral.View):
    ...
    def __init__(self, scene, topic):
        ...
        spyral.event.register(
            "TimeMaster.wait.animation.end",
            self.clear_and_loose)
    ...
    def init_animations(self):
        ...
        self.invasion_anim = spyral.Animation(
            "angle",
            spyral.easing.Linear(
                0,
                math.pi * 2),
    ...
    def clear_and_loose(self):
        ...
        spyral.event.unregister(
            "TimeMaster.wait3.animation.end",
            self.repetir2)
    ...
    def question(self):
        ...
        spyral.event.register_multiple(
            "input.keyboard.down.y", [
                dialog.die, self.reset_and_clear])
...
class TimeMaster(spyral.Sprite):
    ...
    def __init__(self, scene):
        ...
        self.offset_anim = spyral.Animation(
            "offset", spyral.easing.Arc(
                (0, 0), 50), duration=5) + spyral.Animation(
            "offset", spyral.easing.LinearTuple(
                (50, 0), (0, 0)), duration=1)
        self.offset_anim.property = "offset"

        spyral.event.register(
            "TimeMaster.offset.animation.end",
            self.endhandler)
...
class Dialogo(spyral.Sprite):
    ...
    def blink(self):
        ...
        anim = spyral.Animation(
            "image", spyral.easing.Iterate([blank, self.image], times=8), duration=3)
        self.visible = True
        self.animate(anim)
...
class Escena(spyral.Scene):
    ...
    def intro0(self):
        self.intro = Dialogo(
            self,
            "Now I must get to my ship. It is at the bottom of the ocean.",
            self.intro1)
    ...
    def intro1(self):
        self.intro = Dialogo(
            self,
            "Listen carefully and use the arrow keys to navigate in the right direction.",
            self.campo.defered_spawn)
        self.titulo.kill()
