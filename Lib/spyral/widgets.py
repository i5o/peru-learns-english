...
class MultiStateWidget(BaseWidget):
    ...
    def __init__(self, form, name, states):
        ...
        # Hack for now; TODO need to be able to set properties on it even
        ...
        **self._image_locations** internal variabel) and sets the widget's image
        ...
        for state in self._states:
            if self._nine_slice:
                ...
                self._images[state] = spyral.Image(
                    self._image_locations[state])
    ...
    def __stylize__(self, properties):
        ...
        for state in self._states:
            ...
            self._image_locations[state] = properties.pop(
                'image_%s' %
                (state,))
...
class ButtonWidget(MultiStateWidget):
    ...
    def __init__(self, form, name, text="Okay"):
        MultiStateWidget.__init__(
            self, form, name, [
                'up', 'down', 'down_focused', 'down_hovered', 'up_focused', 'up_hovered'])
    ...
    def _on_state_change(self):
        ...
        A function triggered whenever this widget changes size.
        """
        self._text_sprite.pos = spyral.util._anchor_offset(
            self._anchor,
            self._padding[0] /
            2,
            self._padding[1] /
...
class TextInputWidget(BaseWidget):
    ...
    def _render_text(self):
        ...
        if self._selecting and (self._cursor_pos != self._selection_pos):
            ...
            pre_missed = self.font.get_size(
                self._value[
                    :end])[0] - pre.width - highlight.width + 1
            if self._value[:start]:
                ...
                self._rendered_text = spyral.image.from_sequence(
                    (pre, highlight, post), 'right', [
                        pre_missed, post_missed])
            else:
                post_missed = self.font.get_size(
    ...
    def _move_rendered_text(self):
        ...
        if self._view_x + \
                self.box_width > max_width and max_width > self.box_width:
    ...
    def _delete(self, by_word=False):
        ...
        if self._selecting:
            ...
            start = self.cursor_pos
                ...
                self.value, self.cursor_pos, len(
                    self._value))
            self._remove_char(start, end)
        else:
    ...
    def _backspace(self, by_word=False):
        ...
        if self._selecting:
            ...
            start = self._find_previous_word(
                self.value,
                0,
                self.cursor_pos -
                1)
    ...
    def _handle_key_down(self, event):
        ...
        if not shift_is_down or (
                shift_is_down and key not in TextInputWidget._non_insertable_keys):
            self._selecting = False
            self._render_text()
        if self._selecting:
            self._render_text()
